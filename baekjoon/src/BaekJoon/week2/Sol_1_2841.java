...
public class Sol_1_2841 {
    ...
    public static void main(String[] args) {
        ...
        int count = Integer.parseInt(sc.next()); // 음의 수
        int frets = Integer.parseInt(sc.next()); // 프렛의 수

        int result = 0;
        Stack<Integer>[] st = new Stack[count];
        ...
        for (int i = 1; i <= count; i++) {
            /*System.out.println("회차 : " + i + "/" + "count : " + count);
            System.out.println("result : " + result);*/
            line = Integer.parseInt(sc.next());
            fret = Integer.parseInt(sc.next());
            ...
            if (st[line].isEmpty()) {
                ...
                if (st[line].peek() < fret) {
                    ...
                    while (!st[line].isEmpty() && st[line].peek() > fret) {
                        st[line].pop();
                        result++;
                    }

                    if (!st[line].isEmpty() && st[line].peek() < fret) {
                        st[line].push(fret);
                        result++;
                    } else if (st[line].isEmpty()) {
                        st[line].push(fret);
                        result++;
                    }
                }
            }
            /*System.out.println(i + "회차 END : " + " RESULT : " + result);*/
        }
