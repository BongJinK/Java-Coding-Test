...
public class Sol_3_1269 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        Map<String, Integer> groupA = new HashMap<>();
        Map<String, Integer> groupB = new HashMap<>();
        ...
        for (int i = 0; i < n; i++) {
            groupA.put(token.nextToken(), 1);
        }

        token = new StringTokenizer(br.readLine());
        for (int i = 1; i <= m; i++) {
            String b = token.nextToken();
            if (!groupA.containsKey(b)) {
                groupB.put(b, 1);
            } else {
                groupA.remove(b);
            }
        }
        System.out.println(groupA.size() + groupB.size());
        br.close();
    }
