...
public class Sol_7_1406 {
    ...
    public static void main(String[] args) throws IOException {
        // java11 : 0.3초, java8 : 2초
        // 빠른 출력 위해 BufferedWriter 사용
        BufferedReader br = new BufferedReader(new InputStreamReader(System.in));
        BufferedWriter bw = new BufferedWriter(new OutputStreamWriter(System.out));
        Deque<Character> leftSide = new ArrayDeque<>();
        Deque<Character> rightSide = new ArrayDeque<>();

        for (char a : br.readLine().toCharArray()) {
            leftSide.offerLast(a);
        }
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (command == 'L') {
                if (!leftSide.isEmpty()) rightSide.offerFirst(leftSide.pollLast());
            } else if (command == 'D') {
                if (!rightSide.isEmpty()) leftSide.offerLast(rightSide.pollFirst());
            } else if (command == 'B') {
                if (!leftSide.isEmpty()) leftSide.pollLast();
            } else if (command == 'P') {
                leftSide.offerLast(comm.charAt(2));
            }
        }
